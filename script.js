...
document.getElementById('rsvpFormCombined').addEventListener('submit', function(e) {
  ...
  (async () => {
    ...
    if (!name || !wish) {
      showToast('Sila lengkapkan nama dan ucapan.');
      return;
    }
    ...
    try {
      ...
      showToast(`Terima kasih ${name}! Ucapan anda telah disimpan.`);
      this.reset();
    } catch (err) {
      console.error(err);
      showToast('Gagal menyimpan ucapan. Sila cuba semula.');
    }
  })();
});

// Load messages from Pages Functions (D1) and render into wishesContainer
async function loadMessages() {
  try {
    ...
    if (!container) return;
    // clear existing dynamic entries but keep any static/example card
    // Remove all children except the first example card (if you want to keep it)
    container.innerHTML = '';

    data.forEach(entry => {
      const card = document.createElement('div');
      card.className = 'bg-pink-50 p-4 md:p-5 rounded-2xl border border-pink-100 mb-4 toast-enter shadow-sm';
      const nameLine = `${entry.name || 'Tetamu'}`;
      card.innerHTML = `<p class="font-bold text-amber-800 text-[11px] md:text-sm">${nameLine}</p><p class="text-stone-600 mt-1 italic text-[10px] md:text-xs leading-relaxed">"${entry.message || ''}"</p>`;
